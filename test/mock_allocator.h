...
struct monitored_allocator: allocator<T> {
    ...
    monitored_allocator() {
        ...
        ON_CALL(*_monitor, void_allocate)
            .WillByDefault(alloc);
        ON_CALL(*_monitor, void_deallocate)
            .WillByDefault(dealloc);
    }


    monitored_allocator(monitored_allocator&&):
        monitored_allocator{}
    {}
    monitored_allocator(const monitored_allocator&):
        monitored_allocator{}
    {}
    template <typename U>
    monitored_allocator(monitored_allocator<U>&& other):
        monitored_allocator{}
    {}
    template <typename U>
    monitored_allocator(const monitored_allocator<U>&):
        monitored_allocator{}
    {}
    ...
    void deallocate(T* p, std::size_t n)
    {
        _monitor->void_deallocate(p, n * sizeof(T));
    }
