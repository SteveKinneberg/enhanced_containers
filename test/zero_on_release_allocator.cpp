  ...
  protected:
    ...
    std::string report_memory(std::size_t offset, std::size_t len)
    {
        ...
        for (std::size_t i = 0; i < data.size(); ++i) {
            ...
            } else if (data[i] != static_cast<std::byte>(0) && current_value_type != value_type::NON_ZERO) {
                if (r.empty()) {
        ...
        r = fmt::format("{}{})\nExpected zeored byte range to be [{}, {}): {:02x}", r, data.size() - 1,
                        offset, offset + len,
...
TEST_F(zero_on_release_allocator_test, one_element)
{
    ...
    auto& data = memory->get_memory_array();
    {
        ...
        EXPECT_CALL(*this->mock_allocator, void_allocate(_))
            .WillOnce([&len, this](std::size_t n)
            {
                len += n;
